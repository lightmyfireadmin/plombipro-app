...
# Plumbing-related categories to scrape
PLUMBING_CATEGORIES = [
    {'name': 'Tuyauterie', 'url': '/c/plomberie/tuyauterie/p/1001'},
    {'name': 'Raccords', 'url': '/c/plomberie/raccords/p/1002'},
    {'name': 'Robinetterie', 'url': '/c/plomberie/robinetterie/p/1003'},
    {'name': 'Sanitaire', 'url': '/c/plomberie/sanitaire/p/1004'},
    {'name': 'Chauffage', 'url': '/c/plomberie/chauffage/p/1005'},
    {'name': 'Évacuation', 'url': '/c/plomberie/evacuation/p/1006'},
    {'name': 'Outils plomberie', 'url': '/c/outillage/plomberie/p/2001'},
    {'name': 'Raccords PVC', 'url': '/c/plomberie/raccords-pvc/p/1007'},
    {'name': 'Raccords cuivre', 'url': '/c/plomberie/raccords-cuivre/p/1008'},
    {'name': 'Chauffe-eau', 'url': '/c/plomberie/chauffe-eau/p/1009'},
]

class PointPScraper:
    """
    Robust Point P scraper with anti-blocking measures
    """

    def __init__(self, base_url='https://www.pointp.fr'):
    ...
    def parse_product_card(self, product_elem, category):
        ...
        try:
            ...
            desc_selectors = ['.description', '.product-description', '[itemprop="description"]']
    ...
    def scrape_all_categories(self, categories=None, max_categories=None):
        ...
        logger.info(f"Starting scrape of {len(categories)} categories from Point P")
...
def scrape_point_p(cloud_event):
    """Cloud Function entry point for Point P scraping"""
    try:
        logger.info("=== Point P Scraper Started ===")

        scraper = PointPScraper()
        ...
        logger.error(f"Fatal error in Point P scraper: {e}", exc_info=True)
        return {
            'success': False,
            'error': str(e),
            'timestamp': datetime.utcnow().isoformat(),
...
def scrape_point_p_http(request):
    """HTTP endpoint for testing Point P scraper"""
    try:
        # Mock cloud event
        class MockEvent:
            def __init__(self):
        ...
        result, status_code = scrape_point_p(MockEvent())
