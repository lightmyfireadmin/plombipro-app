-- =============================================================================
-- COMPLETE TEMPLATES MIGRATION FOR PLOMBIPRO
-- Created: January 2025
-- Purpose: Create templates table, set up RLS policies, load 50 system templates
--
-- CRITICAL FIX: Previous RLS policies prevented users from seeing system templates
-- because system templates have user_id = NULL. This migration fixes that.
-- =============================================================================

-- Step 1: Create templates table (idempotent - will skip if exists)
CREATE TABLE IF NOT EXISTS public.templates (
    id uuid NOT NULL DEFAULT gen_random_uuid(),
    user_id uuid,
    template_name text NOT NULL,
    template_type text DEFAULT 'quote'::text,
    category text,
    line_items jsonb,
...
CREATE INDEX IF NOT EXISTS idx_templates_is_system ON public.templates(is_system_template);
CREATE INDEX IF NOT EXISTS idx_templates_category ON public.templates(category);
CREATE INDEX IF NOT EXISTS idx_templates_template_type ON public.templates(template_type);
CREATE INDEX IF NOT EXISTS idx_templates_user_system ON public.templates(user_id, is_system_template);

-- Step 3: Enable RLS
ALTER TABLE public.templates ENABLE ROW LEVEL SECURITY;

-- Step 4: Drop existing policies (if any) to avoid conflicts
DROP POLICY IF EXISTS "Users can only see their own templates." ON public.templates;
...
DROP POLICY IF EXISTS "Everyone can see system templates." ON public.templates;
DROP POLICY IF EXISTS "System templates are read-only." ON public.templates;
...
-- Step 5: Create NEW RLS policies (FIXED to allow system templates)
-- Policy 1: Users can see their own templates AND all system templates
CREATE POLICY "templates_select_policy" ON public.templates
    FOR SELECT
    USING (
        auth.uid() = user_id OR -- User's own templates
        is_system_template = true -- OR system templates (available to everyone)
    );

-- Policy 2: Users can only insert their own templates (not system templates)
...
-- Policy 3: Users can only update their own templates (not system templates)
...
-- Policy 4: Users can only delete their own templates (not system templates)
...
-- Step 6: Add comment
COMMENT ON TABLE public.templates IS 'Quote and invoice templates. System templates (is_system_template=true) are visible to all users via RLS policy.';

-- Step 7: Clean up any existing templates to avoid duplicates
-- (Only delete system templates, keep user templates)
DELETE FROM public.templates WHERE is_system_template = true;

-- =============================================================================
-- Step 8: Load 50 System Templates
-- =============================================================================
...
-- =============================================================================
-- Step 9: Verification and Success Messages
-- =============================================================================
...
BEGIN
    ...
    RAISE NOTICE '========================================';
    RAISE NOTICE '';
    RAISE NOTICE '✅ Successfully created % system templates', template_count;

    IF template_count = 0 THEN
        RAISE WARNING '⚠️  No templates were created. Check for errors above.';
    ELSIF template_count < 50 THEN
        RAISE WARNING '⚠️  Only % templates created. Expected 50.', template_count;
    ELSE
...
BEGIN
    ...
    FOR category_rec IN
        ...
        ORDER BY category
    LOOP
        RAISE NOTICE '   • %: % templates', category_rec.category, category_rec.count;
    END LOOP;
END $$;

-- Test RLS: Verify system templates are visible
DO $$
DECLARE
    template_count INTEGER;
BEGIN
    -- This should work even without auth.uid() because of the RLS policy
    ...
    IF template_count > 0 THEN
        RAISE NOTICE '';
        RAISE NOTICE '✅ System templates are visible (RLS configured correctly)';
        RAISE NOTICE '✅ Templates table: CREATED';
        RAISE NOTICE '✅ RLS policies: CONFIGURED';
        RAISE NOTICE '✅ System templates: LOADED (% templates)', template_count;
        RAISE NOTICE '✅ Indexes: CREATED';
    ELSE
        RAISE WARNING '⚠️  System templates might not be visible. Check RLS policies.';
    END IF;
END $$;
...
RAISE NOTICE '========================================';
RAISE NOTICE '1. Refresh your PlombiPro app';
RAISE NOTICE '2. Go to: Devis → Nouveau Devis';
RAISE NOTICE '3. Click: "Sélectionner un modèle"';
RAISE NOTICE '4. You should see 50 templates organized by 7 categories!';
RAISE NOTICE '';
RAISE NOTICE 'Categories:';
RAISE NOTICE '• Rénovation Salle de Bain (10 templates)';
RAISE NOTICE '• Plomberie Cuisine (8 templates)';
RAISE NOTICE '• Chauffe-eau (7 templates)';
RAISE NOTICE '• Dépannage Urgence (6 templates)';
RAISE NOTICE '• Installation Équipements (8 templates)';
RAISE NOTICE '• Chauffage (5 templates)';
RAISE NOTICE '• Équipements Spéciaux (6 templates)';
RAISE NOTICE '';
