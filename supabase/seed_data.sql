...
-- Link company to editionsrevel@gmail.com

-- ============================================
-- 1. TEST USER & PROFILE
-- ============================================

-- Insert test user profile (assumes auth.users already created via Supabase Auth)
-- User email: editionsrevel@gmail.com
-- Note: The user must be created through Supabase Auth first, this just adds profile data
...
(
  ...
  'Dépose et pose nouveaux équipements',
  'draft',
  NOW() + INTERVAL '45 days',
  3245.00,
  649.00,
  3894.00,
  'Projet prévu pour mars 2025 - Durée 5 jours',
  '[
...
(
  ...
  'FAC-2025-002',
  'Contrat maintenance mensuel - Janvier',
  'Entretien chaudières et contrôles',
  'sent',
  NOW() + INTERVAL '30 days',
  520.00,
  104.00,
  ...
  NULL,
  NULL,
  'Paiement attendu avant fin de mois',
  '[
...
(
  ...
  5240.00,
  'Chantier terminé - client satisfait',
...
(
  ...
  'Commander matériaux',
  'Commander tous les équipements sanitaires',
  'todo',
  'high',
  NOW() + INTERVAL '15 days',
  NOW()
...
(
  ...
  'Remplacement colonne eau froide',
  'completed',
  'high',
  NOW() - INTERVAL '5 days',
  NOW() - INTERVAL '10 days'
...
(
  ...
  'Remplacement colonne eau chaude',
  'in_progress',
  'high',
  NOW() + INTERVAL '5 days',
  NOW() - INTERVAL '8 days'
...
('00000000-0000-0000-0000-000000000001'::uuid, 'Nouveau paiement reçu', 'Paiement de 171,00 € reçu pour FAC-2025-001', 'payment', true, '/invoices', NOW() - INTERVAL '5 days'),
...
('00000000-0000-0000-0000-000000000001'::uuid, 'Facture en attente', 'La facture FAC-2025-002 est en attente de paiement', 'reminder', false, '/invoices', NOW() - INTERVAL '1 day'),
...
(
  ...
  30,
  20.0,
  'Jean Revel - Gérant - Plomberie Revel',
  NOW(),
  NOW()
